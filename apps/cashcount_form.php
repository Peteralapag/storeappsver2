...
if($transmode == 'edit')
{
	...
	$dinomination  = $functions->GetEditInfo($table,$rowid,'denomination',$db);
	$quantity  = $functions->GetEditInfo($table,$rowid,'quantity',$db);	
	$total_amount = $functions->GetEditInfo($table,$rowid,'total_amount',$db);
}
if($transmode == 'new')
{
	$rowid = '';
...
<table style="width: 100%" class="tables" cellpadding="4" cellspacing="15">
	...
	<tr>
		<td>
			<input id="dinomination" type="number" class="form-control form-input" placeholder="Dinomination Ex: 100" value="<?php echo $dinomination; ?>" autocomplete="no"></td>
		<td style="width:10px;"></td>
		<td>
			<input id="quantity" type="number" class="form-control form-input" value="<?php echo $quantity; ?>" autocomplete="no">
		</td>
		<td style="width:10px;"></td>
		<td>
			<input id="total_amount" type="number" class="form-control form-input" value="<?php echo $total_amount; ?>" autocomplete="no" disabled>
		<td style="width:10px;"></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		...
		<td colspan="3" style="text-align:right">
			...
			<button class="btn btn-success btnnew" style="width:auto" onclick="commitAction('new')">Save Cash Count</button>
			<?php } if($transmode == 'edit') { ?>
			<button class="btn btn-info btnupdate" onclick="commitAction('edit')" style="width: auto">Update Cash Count</button>
			<?php } ?>
...
function commitAction(transmode)
{
	...
	var dinomination = $('#dinomination').val();
	...
	else if(dinomination  == '') { app_alert("Donimation Error","Dinomination must not be empty.","warning","Ok","uom","focus"); return false; }
	else if(totalamount == '') { app_alert("Total Amount Error","Total amount is empty","warning","Ok","suppprefix","focus"); return false; }
	
	if(transmode === 'new')
	{ 
		var mode = 'savecashcount'; 
