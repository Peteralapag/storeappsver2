...
if($transmode == 'edit')
{
	...
	$time = $functions->GetEditInfo($table,$rowid,'inputtime',$db);
	...
	$kilo_used  = $functions->GetEditInfo($table,$rowid,'kilo_used',$db);
	$actual_yield  = $functions->GetEditInfo($table,$rowid,'actual_yield',$db);
	$standard_yield  = $functions->GetEditInfo($table,$rowid,'standard_yield',$db);
...
<table style="width: 850px" class="tables" cellpadding="4" cellspacing="15">
	...
	<tr>
		...
		<td style="width:10px;"></td>
		<td><input id="kilo_used" type="text" class="form-control form-input" value="<?php echo $kilo_used; ?>" autocomplete="no"></td>
	</tr>
	<tr>
		<th>ACTUAL YIELD</th>
		<th style="width:10px;"></th>
	...
	<tr>
		<td>
			<input id="standard_yield" type="hidden" value="<?php echo $standard_yield; ?>">
			<input id="actual_yield" type="text" class="form-control form-input" placeholder="Actual Yield" value="<?php echo $actual_yield; ?>" autocomplete="no">
		</td>
		<td style="width:10px;"></td>
		<td>
	...
	<tr>
		...
		<td colspan="3" style="text-align:right">			
			...
			<button class="btn btn-success btnnew" onclick="commitAction('new')" disabled>Save FGTS</button>
			<?php } if($transmode == 'edit') { ?>
			<button class="btn btn-info btnupdate" onclick="commitAction('edit')">Update FGTS</button>
			<?php } ?>
...
function commitAction(transmode)
{
	...
	var standard_yield = $('#standard_yield').val();
	...
	else if(kilo_used <= '0') { app_alert("Kilo Used Error","Kilo used cannot be empty or equal to zero.","warning","Ok","kilo_used","focus"); return false; }
	else if(actual_yield <= '0' || actual_yield <= '0.00') { app_alert("Actual Yield Error","Actual Yield cannot be empty or equal to zero.","warning","Ok","actual_yield","focus"); return false; }
