...
<?php
$query ="SELECT * FROM store_frozendough_data $q ORDER BY id DESC";  
...
?>	
	<tbody>	
		<tr <?php echo $tr_command; ?> <?php echo $noid_class; ?>>
			...
			<td style="text-align:center"><?php echo $ROWS['inputtime']; ?></td> <!-- TIME -->
			<td><?php echo $ROWS['item_name']." ".$noid_text; ?></td> <!-- ITEM NAME -->
			<td style="text-align:right; padding-right:30px !important"><?php echo $ROWS['kilo_used']; ?></td> <!-- ITEM NAME -->
			<td style="text-align:right; padding-right:30px !important"><?php echo $ROWS['actual_yield']; ?></td> <!-- ACTUAL YIELD -->
			<td><?php echo $ROWS['shift']; ?></td> <!-- SHIFT -->
			<td><?php echo $ROWS['employee_name']; ?></td> <!-- BAKE -->
			<td><?php echo $ROWS['slip_no']; ?></td>
			<td style="text-align:center"><?php echo $status; ?></td>
			<td style="text-align:center; padding:1px !important" class="actions">
				<?php if($status == 'Open') { ?>
				<div>
					<!--button class="btn btn-success btn-sm" style="width:49%;font-size:11px" onclick="editItem('edit','<?php echo $file_name; ?>','<?php echo $title; ?>','<?php echo $rowid; ?>')"><i class="fa-duotone fa-pencil"></i></button-->
					...
					<!--button class="btn btn-danger btn-sm" style="width:49%;font-size:11px" onclick="deleteItem('<?php echo $rowid; ?>','fgts','<?php echo $item_name; ?>')"><i class="fa-solid fa-trash"></i></button-->
					<button class="btn btn-danger btn-sm" style="width:49%;font-size:11px" onclick="DeleteUserChecking('<?php echo $rowid; ?>','frozendough','<?php echo $item_name; ?>')"><i class="fa-solid fa-trash"></i></button>
				</div>
				<?php }  else {?>
				<div>
