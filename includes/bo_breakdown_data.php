...
<div class="container-fluid">
	<table id="upper" class="table table-striped table-bordered">
		<tr>
			<th colspan="22">BO BREAKDOWN - <?php echo $monthname.' - '.$yearname;?></th>
		</tr>
		<tr>
			<th>DATE</th>
	...
	<?php
	    ...
	    echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE CLASSIC HOT BREAD',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE HIGH-END BREADS',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE BINALOT & PASALUBONG',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE NUTRI-DENSE',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE TASTY LOAF',$selectedbranch,$reportdate,$db),2).'</td>';
		
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ROSE CLASSIC SPECIAL',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('ALL TIME CAKE (CLASSIC)',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('CELEBRATION CAKES (FLAGSLIP)',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('PREMIUM CAKES (HIGH-END)',$selectedbranch,$reportdate,$db),2).'</td>';
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('HIGH-END COFFEE',$selectedbranch,$reportdate,$db),2).'</td>';
		
		echo '<td style="text-align: right">'.number_format($TheFunctions->boBreakdown('FGFRAP',$selectedbranch,$reportdate,$db),2).'</td>';
...
?>		
	<tr>
		...
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ROSE CLASSIC HOT BREAD',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo $TheFunctions->boBreakdownTotalPerMonthandDate('ROSE HIGH-END BREADS',$selectedbranch,$monthNumbers,$yearname,$db);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ROSE BINALOT & PASALUBONG',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ROSE NUTRI-DENSE',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ROSE TASTY LOAF',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ROSE CLASSIC SPECIAL',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('ALL TIME CAKE (CLASSIC)',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('CELEBRATION CAKES (FLAGSLIP)',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('PREMIUM CAKES (HIGH-END)',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('HIGH-END COFFEE',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
		
		<td style="text-align: right; font-weight:bold"><?php echo number_format($TheFunctions->boBreakdownTotalPerMonthandDate('FGFRAP',$selectedbranch,$monthNumbers,$yearname,$db),2);?></td>
